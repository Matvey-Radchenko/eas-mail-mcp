...
`mail_list` samples per server, with 100 generated messages per call. The Python
script is the reference implementation. The production application is inspected
for binary size; measured startup, RSS, and mail latency belong to the Rust
synthetic server, not a live Exchange account.
